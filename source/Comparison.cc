...
void OrderMaker::queryOrderMaker(const OrderMaker& sortOrder, const CNF& query, OrderMaker& queryorder, OrderMaker& cnforder) {
	queryorder.numAtts = cnforder.numAtts = 0;
	for (size_t _ii=0; _ii<(sortOrder.numAtts); ++_ii) {
		typeof(sortOrder.whichAtts[_ii]) & att = sortOrder.whichAtts[_ii];
		typeof(sortOrder.whichTypes[_ii]) & type = sortOrder.whichTypes[_ii];
		int attributeIndx = -1;
		for(size_t i=0; i<(query.numAnds); ++i) {
			typeof(query.orList[i]) & clause = query.orList[i];
			typeof(query.orLens[i]) & orLens = query.orLens[i];
			if (orLens==1) {
		...
		if (attributeIndx>=0) {     // found this attribute in query CNF
			queryorder.whichAtts[queryorder.numAtts]=(att);
			queryorder.whichTypes[queryorder.numAtts]=(type);
			cnforder.whichAtts[cnforder.numAtts] = (attributeIndx);
